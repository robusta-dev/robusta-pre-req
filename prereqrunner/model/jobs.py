...
from prereqrunner.utils import container_resources, containers_resources_sum
from prereqrunner.model.pods import ContainerResources, ResourceAttributes
...
class JobContainer(BaseModel):
    ...
    def from_api_server(container: V1Container) -> "JobContainer":
        requests: ContainerResources = container_resources(container, ResourceAttributes.requests)
        limits: ContainerResources = container_resources(container, ResourceAttributes.limits)
        return JobContainer(
...
class JobInfo(BaseModel):
    ...
    def from_api_server(job: Union[V1Job, Job], pods: List[str]) -> "JobInfo":
        ...
        requests: ContainerResources = containers_resources_sum(containers, ResourceAttributes.requests)
